...
import '../assests/Stylesheets/tiktok.css'
import TikTokMediaDownloader from '../config/TikTokAPI'; // THE API FILE
...
function Tiktok() {
    ...
    const HandleSubmit = async () => {
        ...
        if (trimmedUrl) {
            try {
                ...
                const response = await TikTokMediaDownloader(trimmedUrl); // we are sending the url to the API to check If there is a video from this URL
    ...
    return (
        <>
            <h1 className='tiktok-heading'>TikTok</h1>

            <div className='tt-input-container'>
                <input type="text" className='tt-input' placeholder="Paste video link here..." value={url} onChange={(e) => setURL(e.target.value)} />
            </div>

            <div className='btn-container'>
                <button className='tt-buttons search-btn' onClick={HandleSubmit}>Search</button>
                {data &&
                    <a href={data} download>
                        <button className='tt-buttons download-btn'>Download</button>
                    </a>
                }
            </div>
